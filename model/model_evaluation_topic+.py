...
if __name__ == "__main__":
	...
	for j in range(1, 11):
		recall = recall_CF(top_20, test_data, "recommendation_result_user_based_CF_topic10_" + str(float(j)/10.0) + ".txt")
		for i in range(0, 20):
			recall[i] = float(recall[i]) / float(total_number)
		x = range(1, 21)
		l = "lambda:" + str(float(j)/10.0)
		pl.plot(x, recall, label = l)
	pl.ylabel('recall rate')
        pl.xlabel('recommend K libraries to each repository')
        pl.legend(loc='lower right')
        pl.savefig("userbased_topic_recall1.png")
	pl.close()
	
	for j in range(1, 10):
                recall = recall_CF(top_20, test_data, "recommendation_result_user_based_CF_topic10_" + str(float(j)/100.0) + ".txt")
                for i in range(0, 20):
                        recall[i] = float(recall[i]) / float(total_number)
                x = range(1, 21)
                l = "lambda:" + str(float(j)/100.0)
                pl.plot(x, recall, label = l)

	pl.ylabel('recall rate')
	pl.xlabel('recommend K libraries to each repository')
	pl.legend(loc='lower right')
	pl.savefig("userbased_topic_recall.png")
