...
class CTR:
    def __init__(self, n_user, n_item, ratings, repo_id_name_dict, package_id_name_dict, vector_repo, file_name, lambda_u = 10, n_topic = 200):
	...
	self.lambda_u = lambda_u
        ...
        self.theta = np.zeros([n_user, n_topic])

        for user in ratings:
	    self.theta[user][:] = vector_repo[user]
            for item in ratings[user]:
                self.C[user][item] = self.a
                self.R[user][item] = 1


    def fit(self, max_iter=25):
        old_err = 0
